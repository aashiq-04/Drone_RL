...
# Load DDPG model
MODEL_PATH = "checkpointDDPG/ddpg_final"
...
for episode in range(NUM_EPISODES):
    ...
    for step in range(MAX_STEPS):
        ...
        ax.set_ylabel("Y")
        ax.set_zlabel("Z")
        ax.set_title(f"Drone Navigation with DDPG - Episode {episode + 1}")
        ax.legend()
...
# Save metrics
np.save("ddpg_test_rewards.npy", test_rewards)
np.save("ddpg_test_collisions.npy", test_collisions)
np.save("ddpg_test_steps.npy", test_steps)

print("\n📊 **Overall DDPG Performance** 📊")
