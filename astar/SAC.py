...
#         states = torch.FloatTensor(states)
#         actions = torch.FloatTensor(actions)
#         rewards = torch.FloatTensor(rewards).view(-1, 1)  # Ensure correct shape
#         next_states = torch.FloatTensor(next_states)
#         dones = torch.FloatTensor(dones).view(-1, 1)  # Ensure correct shape
...
#         # log_probs = torch.log(torch.clamp(1 - new_actions.pow(2), min=1e-6))  # Approximate entropy
#         log_probs = -((new_actions**2).sum(dim=-1,keepdim=True))
#         actor_loss = (self.alpha * log_probs - q1_actor).mean() 
...
#     #     # states = torch.FloatTensor(states)
#     #     # actions = torch.FloatTensor(actions)
#     #     states = torch.tensor(states, dtype=torch.float32).unsqueeze(0)  # Ensure batch dim
#     #     actions = self.actor(states).unsqueeze(0)
...
#     #     # Compute current Q estimates
#     #     q1, q2 = self.critic(states, actions)  # Pass states and actions separately
#     #     critic_loss = ((q1 - y) ** 2).mean() + ((q2 - y) ** 2).mean()
...
#     #     # Update policy (Actor)
#     #     q1_actor = self.critic.q1(states, self.actor(states))  # Pass states and actions separately
