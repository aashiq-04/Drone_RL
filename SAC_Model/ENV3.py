...
class DroneEnv3D(gym.Env):
    ...
    def reset(self):
        ...
        attempts = 0
        while attempts < 50:
            self.drone_pos = np.array([random.uniform(0, 9), random.uniform(0, 2), random.uniform(0, 4)], dtype=np.float64)
        ...
        attempts = 0
        while attempts < 50:
            self.target = np.array([random.uniform(2,9), random.uniform(8, 10), random.uniform(4,9)], dtype=np.float64)
    ...
    def check_collision(self,safety_distance=0.25):
        # print(abs(self.dmin)
