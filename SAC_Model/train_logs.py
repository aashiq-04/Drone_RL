...
from CONFIG import MAX_ACTION,MAX_EPISODES,MAX_STEPS,BATCH_SIZE,BUFFER_SIZE,STATE_DIM,ACTION_DIM, LOAD_CHECKPOINT
# Hyperparameters

CHECKPOINT_DIR = "Updated_Checkpoint"
...
for episode in range(1,MAX_EPISODES+1):
    ...
    writer.add_scalar('Reward/Episode', episode_reward, episode)  # Log total episode reward
    writer.add_scalar('Reward/Avg', avg_reward, episode)  # Log average reward per episode
    writer.add_scalar('Collision Rate', collision_percent, episode)  # Log collision rate
    writer.add_scalar('Exploration Efficiency', np.mean(total_exploration_efficiency), episode)  # Log exploration efficiency
    writer.add_scalar('Time Efficiency', np.mean(total_time_efficiency), episode)  # Log time efficiency
